...
def test_create_user(reqres):
    created_user = reqres.post("api/users", {"name": "Olga", "job": "Tester"})

    assert created_user.status_code == 201
    assert S(create_user) == created_user.json()
    assert created_user.json()["name"] == "Olga"
...
def test_update_user_by_put(reqres):
    update_user = reqres.put("api/users/2", {"name": "olarionova", "job": "aqa"})

    assert update_user.status_code == 200
    assert S(create_update_user) == update_user.json()
    assert update_user.json()["name"] == "olarionova"
...
def test_update_user_by_patch(reqres):
    update_user = reqres.put("api/users/2", {"name": "Larionova", "job": "sdet"})

    assert update_user.status_code == 200
    assert S(create_update_user) == update_user.json()
    assert update_user.json()["name"] == "Larionova"
